...
export default function Cart() {
  ...
  const { totalCartPrice, allProducts, deleteProduct, clearCart, getUserCart } = useContext(counterContext);
        ...
        <div className="container">
          {allProducts.map((product, idx) => <div className='row' key={idx}>
            <div className="col-md-1 mb-1">
              <img src={product.product.imageName} alt={product.product.name} className='w-100' />
            </div>
            <div className="col-md-9">
              <div className='d-flex justify-content-start align-items-center'>
                <h5 style={{paddingRight: 5}}>{product.product.name.toUpperCase()}</h5>
                <h5 style={{color: '#ffa200', fontWeight: 'bold'}}>({product.quantity})</h5>
              </div>
              <h5>price : {product.product.price * product.quantity}</h5>
            </div>
            <div className="col-md-2">
              <button className='btn btn-outline-danger' onClick={() => { deleteCartProduct(product.id) }}>Remove  <i className="fa-regular fa-trash-can"></i></button>
