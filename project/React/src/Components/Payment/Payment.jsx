import axios from "axios";
import React, { useContext, useEffect, useState } from "react";
import toast from "react-hot-toast";
import { useFormik } from "formik";
import * as yup from "yup";
import { useNavigate } from "react-router-dom";
import { counterContext } from "../../Context/product";
import MyModal from "../CreditCard/CreditCardForm";
import "bootstrap/dist/css/bootstrap.min.css";
import { tokenContext } from "../../Context/token";

export default function Payment() {
  const [showModal, setShowModal] = useState(false);
  const { clearCart, allProducts } = useContext(counterContext);
  const { token, getPayload } = useContext(tokenContext);
  const nav = useNavigate();

  // async function handleLocation(body) {
  //     try {
  //         axios.post()
  //     } catch (error) {

  //     }
  // }

  async function confrimCashPayment() {
    const details = document.getElementById("details").value;
    const address = document.getElementById("address").value;
    const city = document.getElementById("city").value;
    const shippingData = {
      address,
      city,
      discripthion: details,
    };
    console.log("reached confirm cash function:", shippingData);
    try {
      const res = await axios.post(
        `http://localhost:8090/addOrder/${getPayload(token).id}`,
        shippingData
      );
      toast.success(res.data);
      setTimeout(() => {
        nav("/allorders");
      }, 2000);
    } catch (error) {
      toast.error("Error occured");
    } finally {
      await clearCart();
    }
  }

  function confirmOnlinePayment() {
    const details = document.getElementById("details").value;
    const address = document.getElementById("address").value;
    const city = document.getElementById("city").value;

    const shippingData = {
      address,
      city,
      discripthion: details,
    };
    axios
      .post(
        `http://localhost:8090/addOrderbank/${getPayload(token).id}`,
        shippingData
      )
      .then((res) => {
        toast.success("Payment completed successfully");
        // getUserCart();
        // setTimeout(() => {
        //   nav("/home");
        // }, 2000);
        setTimeout(() => {
          nav("/allOrders");
        }, 2000);
        clearCart();
      })
      .catch((err) => {
        toast.error(err.response.data.message);
      });
  }

  const handleShowModal = () => {
    setShowModal(true);
  };

  const handleCloseModal = () => {
    setShowModal(false);
  };

  // const validateSchema = yup.object({
  //     "name": yup.string().required("Category Name is required")
  //   })

  //   const locationForm = useFormik({
  //     initialValues: {
  //       "name": "",
  //       "imageName": ""
  //     },

  //     validationSchema: validateSchema,
  //     onSubmit: handleLocation
  //   })

  return (
    <>
      <div className="container">
        <h2 className="mt-3  text-center">Payment</h2>
        {/* <form> */}
        <div className="row justify-content-center ">
          <div className="col-md-8">
            <div className="mb-3">
              <label htmlFor="exampleFormControlInput1" className="form-label">
                City
              </label>
              <input
                type="text"
                className="form-control"
                id="city"
                placeholder="Cairo..."
              />
            </div>
          </div>
          <div className="col-md-8">
            <div className="mb-3">
              <label htmlFor="exampleFormControlInput2" className="form-label">
                Address
              </label>
              <input
                type="text"
                className="form-control"
                id="address"
                placeholder="Area..."
              />
            </div>
          </div>
          <div className="col-md-8">
            <div className="mb-3">
              <label
                htmlFor="exampleFormControlTextarea1"
                className="form-label"
              >
                Details
              </label>
              <textarea
                className="form-control"
                id="details"
                rows="3"
                placeholder="address..."
              ></textarea>
            </div>
          </div>
        </div>
        <div className="d-flex justify-content-center text-center gap-2">
          <div className="col-md-3">
            <button
              onClick={confrimCashPayment}
              className="btn btn-primary w-100"
            >
              Confirm Cash Payment <i className="fa-solid fa-coins ps-1"></i>
            </button>
          </div>
          <MyModal
            show={showModal}
            hide={handleCloseModal}
            onClick={confirmOnlinePayment}
          />
          <div className="col-md-3">
            <button className="btn btn-success w-100" onClick={handleShowModal}>
              Confirm Card Payment{" "}
              <i className="fa-regular fa-credit-card ps-1"></i>
            </button>
          </div>
        </div>
        {/* </form> */}
      </div>
    </>
  );
}
